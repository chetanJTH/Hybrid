...
public class Fourteen_Social_Supervision extends Home {
    public Fourteen_Social_Supervision(AndroidDriver driver) {
        super(driver);
    }
    ...
    @Test
    public void FourteenSocialSupervisionTest() throws InterruptedException, NoSuchElementException {
        test = ExtentReport.shared().getExtentTest("14 days web supervision");
...
//                            + "new UiSelector().className(\"" + className + "\"), \"" + text + "\")")).click();
    ...
    public void FourteenDaysHistory() {
        ParentalFactory P = new ParentalFactory(driver);
        if (P.FourteenDays != null) {
            P.FourteenDays.click();
            Log.addLog("14 days button clicked");
            test.log(Status.INFO, "14 days button clicked");
        } else {
            Log.addLog("14 days button not found");
            test.log(Status.FAIL, "14 days button not found");
        }
    }
